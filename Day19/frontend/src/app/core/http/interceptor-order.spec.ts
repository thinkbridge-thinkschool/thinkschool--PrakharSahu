...
describe('interceptor order', () => {
  ...
  function configure(chain: readonly Parameters<typeof withInterceptors>[0][number][]) {
    // TokenStore restores from sessionStorage at construction, so a session left by an
    // earlier spec would switch the refresh interceptor on and change what this one sees.
    clearBrowserState();
