...
describe('QuotesApiClient', () => {
  ...
  // A paging envelope is the most likely contract drift. It must not read as "no quotes".
  it('reports a 200 that is not an array of quotes instead of showing an empty list', async () => {
    TestBed.tick();
    httpTesting.expectOne('/api/quotes').flush({ items: WIRE_RESPONSE, total: 1 });
    await settled();

    expect(client.quotes()).toEqual([]);
    expect(client.failure()?.name).toBe('MalformedQuoteError');
  });

  it('rejects an array whose elements are missing a contract field', async () => {
    TestBed.tick();
    httpTesting.expectOne('/api/quotes').flush([{ id: 1, text: 'x', author: 'y' }]);
    await settled();
