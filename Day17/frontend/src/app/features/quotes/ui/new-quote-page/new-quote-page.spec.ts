...
describe('NewQuotePage', () => {
  ...
  it('posts exactly author and text, trimmed', async () => {
    await fill('  Ada Lovelace  ', '  Signals first.  ');
    await submitForm();

    const request = httpTesting.expectOne('/api/quotes');
    expect(request.request.method).toBe('POST');
    expect(request.request.body).toEqual({ author: 'Ada Lovelace', text: 'Signals first.' });
  ...
  it('refuses to post special characters, and names the ones it found', async () => {
    await fill('Ada #Lovelace', 'Cost me $5!');
    await submitForm();

    httpTesting.expectNone('/api/quotes');
    const summary = root().querySelector('[data-testid="form-errors"]')?.textContent ?? '';
    expect(summary).toContain('Author cannot contain #');
    expect(summary).toContain('Quote text cannot contain $ !');
    expect(router.url).toBe('/quotes/new');
  });

  it('accepts ordinary prose punctuation', async () => {
    await fill("Charlotte Brontë", 'What is it, then? A quote (a short one) - nothing more.');
    await submitForm();

    const request = httpTesting.expectOne('/api/quotes');
    expect(request.request.body).toEqual({
      author: 'Charlotte Brontë',
      text: 'What is it, then? A quote (a short one) - nothing more.',
    });
    request.flush(CREATED, { status: 201, statusText: 'Created' });
  ...
  /**
   * The duplicate rule lives on the server — this page has no copy of the quote list to check
   * against, and a client-side check would be a guess that another tab could invalidate anyway.
   * What the page owes the user is the server's own sentence, not the generic 409 wording.
   */
  it('surfaces the double-post message on a 409 and keeps the typed quote', async () => {
    await fill('Ada Lovelace', 'Signals first.');
    await submitForm();
    httpTesting
      .expectOne('/api/quotes')
      .flush(
        { message: 'This author has already posted that quote. The same words credited to a different author are fine.' },
        { status: 409, statusText: 'Conflict' },
      );
    await settle(3);

    expect(root().querySelector('[data-testid="create-failed"]')?.textContent).toContain(
      'This author has already posted that quote',
    );
    // The form must not reset: the fix is to change the author, not to retype everything.
    expect(root().querySelector<HTMLInputElement>('#create-quote-author')!.value).toBe(
      'Ada Lovelace',
    );
    expect(router.url).toBe('/quotes/new');
  });
