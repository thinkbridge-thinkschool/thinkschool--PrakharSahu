...
using QuotesApi.Identity.Contracts;
...
public static class AuthEndpoints
{
    ...
    private static void AppendRefreshCookie(HttpResponse response, string refreshToken)
    {
        response.Cookies.Append(RefreshCookieName, refreshToken, new CookieOptions
        {
            ...
            Path = ApiVersioning.AuthCookiePath,
    ...
    {
        // -----------------------------------------------------------------------------------
        // Day 27: anonymous by necessity, and rate-limited because of it.
        //
        // These four endpoints are the only ones that CANNOT require a token — you cannot
        // present a credential to the endpoint that issues credentials. That makes them the
        // most exposed surface in the API and the one place the deny-by-default fallback policy
        // has to be waived explicitly.
        //
        // `AllowAnonymous` is therefore paired with the tight limiter rather than left bare.
        // Five attempts per minute per caller: enough for a human who mistypes a password,
        // useless for credential stuffing. It also protects the SERVER, because BCrypt is
        // deliberately slow and an unthrottled login endpoint is a CPU exhaustion vector as much
        // as a brute-force one.
        //
        // The policy goes on the GROUP, so /refresh and /logout inherit it. A refresh token is a
        // credential too, and an attacker who has one but not the access token would otherwise
        // have an unthrottled endpoint to exchange it at.
        // -----------------------------------------------------------------------------------
        var group = app.MapGroup("/auth")
                       .AllowAnonymous()
                       .RequireRateLimiting(RateLimitPolicies.Auth);

        group.MapPost("/register", async ([FromBody] RegisterRequest request, HttpResponse response, AppDbContext db, IOptions<JwtOptions> jwtOptions, ILogger<IdentityLog> logger, CancellationToken ct) =>
        {
            ...
            return Results.Created($"{ApiVersioning.Prefix}/users/{user.Id}", session);
        });
        ...
        group.MapPost("/refresh", async (HttpRequest httpRequest, HttpResponse response, [FromBody] RefreshRequest? request, AppDbContext db, IOptions<JwtOptions> jwtOptions, ILogger<IdentityLog> logger, CancellationToken ct) =>
        {
