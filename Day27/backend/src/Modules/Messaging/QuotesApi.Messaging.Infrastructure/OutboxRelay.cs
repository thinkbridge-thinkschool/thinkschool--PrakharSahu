...
using QuotesApi.SharedKernel.Observability;
...
public sealed class OutboxRelay : BackgroundService
{
    ...
    private async Task<int> SweepAsync(CancellationToken cancellationToken)
    {
        ...
        foreach (var message in claimable)
        {
            ...
            // ---------------------------------------------------------------------------------
            // Day 26: RE-PARENT this publish onto the request that caused it.
            //
            // This is the other half of what OutboxWriter stored, and the line that makes the
            // whole trace one trace instead of two.
            //
            // `parentId` takes a W3C traceparent string rather than an Activity, which is exactly
            // what is needed here: the parent Activity is long gone — it ended when the HTTP
            // response was written, possibly in another process, possibly minutes ago. Only its
            // identity survived, on the row, and an identity is all the linkage needs.
            //
            // When TraceParent is null the overload without it is used, which starts a ROOT span.
            // That is the correct fallback: a message with no origin genuinely has no parent, and
            // inventing one would attach it to whatever unrelated sweep happens to be running.
            //
            // ActivityKind.Producer because this span *sends*. App Insights renders Producer and
            // Consumer spans as the two ends of a queue hop, which is what draws the arrow from
            // the worker to the subscription consumers in the transaction view.
            //
            // Everything downstream follows automatically from this one line: the Service Bus SDK
            // reads Activity.Current when the message is sent and writes its traceparent into the
            // message's application properties, so the consumer — a different process again —
            // picks the context back up with no code of ours involved.
            // ---------------------------------------------------------------------------------
            using var publishActivity = string.IsNullOrEmpty(message.TraceParent)
                ...
                : Telemetry.Outbox.StartActivity(
                    "outbox.publish",
                    ActivityKind.Producer,
                    parentId: message.TraceParent);

            // Tags, not log lines. A tag is queryable in KQL as a column on the span; the same
            // fact written into a log message is a substring somebody has to parse at 3am.
            publishActivity?.SetTag("outbox.message_id", message.Id);
            publishActivity?.SetTag("outbox.event_type", message.Type);
            publishActivity?.SetTag("outbox.attempt", message.Attempts + 1);
            ...
            try
            {
                ...
                // Day 26 metrics. Lag is measured from OccurredAt — when the domain event
                // happened — rather than from when this sweep claimed the row, because the
                // question worth answering is "how stale is the event a consumer just received",
                // not "how fast was the last hop".
                var lagSeconds = (message.ProcessedAt.Value - message.OccurredAt).TotalSeconds;
                Telemetry.OutboxPublished.Add(1, new KeyValuePair<string, object?>("event_type", message.Type));
                Telemetry.OutboxLagSeconds.Record(lagSeconds, new KeyValuePair<string, object?>("event_type", message.Type));
                publishActivity?.SetTag("outbox.lag_seconds", Math.Round(lagSeconds, 3));
