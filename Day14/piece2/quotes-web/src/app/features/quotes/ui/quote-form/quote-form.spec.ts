...
describe('QuoteForm accessibility and states', () => {
  ...
  // Deliberately the same assertions as quote-form-reactive.spec.ts, so any behavioural
  // difference between the two APIs shows up as a failure rather than as an opinion.
  describe('pristine / dirty / touched', () => {
    ...
    it('starts pristine, untouched and invalid', () => {
      expect(readState('sf-pristine')).toBe('true');
      expect(readState('sf-dirty')).toBe('false');
      expect(readState('sf-touched')).toBe('false');
      expect(readState('sf-valid')).toBe('false');
    });

    it('becomes dirty on input but stays untouched until blur', async () => {
      await type(authorInput(), 'A');

      expect(readState('sf-dirty')).toBe('true');
      expect(readState('sf-pristine')).toBe('false');
      expect(readState('sf-touched')).toBe('false');
    });
    ...
    it('becomes touched on blur without becoming dirty', async () => {
      ...
      expect(readState('sf-touched')).toBe('true');
      expect(readState('sf-dirty')).toBe('false');
    });
    ...
    it('turns valid once both fields satisfy the server rules', async () => {
      ...
      expect(readState('sf-valid')).toBe('true');
    });
