...
import {
  ALLOWED_PUNCTUATION_HINT,
  QUOTE_AUTHOR_MAX_LENGTH,
  QUOTE_TEXT_MAX_LENGTH,
  disallowedCharactersIn,
  isBlank,
  serverLengthOf,
...
/**
 * Names the characters the server would reject, rather than saying "invalid input" and
 * leaving the user to bisect their own sentence. Same allow-list as `TextRules.cs`, so a
 * value that passes here also passes `Quote.Create` — this is a nicer error, not a new rule.
 */
function noSpecialCharacters(label: string) {
  return ({ value }: { value: () => string }): ValidationError | undefined => {
    const offenders = disallowedCharactersIn(value());
    return offenders.length > 0
      ? {
          kind: 'charset',
          message:
            `${label} cannot contain ${offenders.join(' ')}. ` +
            `Use letters, numbers, spaces and ${ALLOWED_PUNCTUATION_HINT} only.`,
        }
      : undefined;
  };
}
...
export const createQuoteSchema = schema<CreateQuoteModel>((path) => {
  ...
  validate(path.author, noSpecialCharacters('Author'));
  ...
  validate(path.text, noSpecialCharacters('Quote text'));
});
...
export class CreateQuoteStore {
  ...
  readonly allowedPunctuation = ALLOWED_PUNCTUATION_HINT;
  ...
  private explain(failure: unknown): string {
    ...
    switch (failure.kind) {
      ...
      default:
        // 400 and 409 both arrive as the server's own DomainError sentence — e.g. "Text must
        // be between 1 and 1000 characters." or "This author has already posted that quote."
        // Those are more specific than anything that could be written here, and `toApiError`
        // already guarantees the body was JSON rather than a stack trace.
