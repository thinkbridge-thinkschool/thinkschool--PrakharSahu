...
// ---------------------------------------------------------------------------------------------
// The name is arbitrary metadata — only the 0.0.0.0-0.0.0.0 pair carries meaning. Day 23 used
// Azure's conventional `AllowAllWindowsAzureIps`, which deploys perfectly well but trips azd's
// reserved-word pre-check on the substring "Windows":
//
...
//
// That warning is wrong — the rule deployed on the first run, while a genuine bug in the same
// template went unmentioned. It is renamed here because a warning that cries wolf on every
// deployment is worse than no warning: the next one gets skimmed too.
